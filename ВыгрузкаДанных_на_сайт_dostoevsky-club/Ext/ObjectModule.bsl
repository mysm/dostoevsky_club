...
Функция ВыгрузитьСувениры() Экспорт
	...
	Попытка
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               |	НоменклатураСпр.Ссылка КАК Guid,
		               |	ЕСТЬNULL(Сув_НаименованиеДляСайта.Значение.Наименование, ""null"") КАК НаименованиеДляСайта,
		               |	НоменклатураСпр.НаименованиеПолное КАК НаименованиеПолное,
		               |	НоменклатураСпр.Артикул КАК Артикул,
		               |	НоменклатураСпр.Производитель КАК Производитель,
		               |	ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних1.Цена, 0) КАК ЦенаСоСкидкой,
		               |	ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0) КАК Цена,
		               |	НоменклатураСпр.Описание КАК Описание,
		               |	ЕСТЬNULL(Сув_Измерение1.Значение, ""null"") КАК Измерение1,
		               |	ЕСТЬNULL(Сув_Измерение2.Значение, ""null"") КАК Измерение2,
		               |	ЕСТЬNULL(Сув_Измерение3.Значение, ""null"") КАК Измерение3,
		               |	ЕСТЬNULL(Сув_Вес.Значение, ""null"") КАК Вес,
		               |	ЕСТЬNULL(Сув_СтранаПроизводитель.Значение.Ссылка, ""null"") КАК СтранаПроизводитель,
		               |	ЕСТЬNULL(Сув_is_active.Значение, ""null"") КАК is_active,
		               |	ЕСТЬNULL(Сув_is_available.Значение, ""null"") КАК is_available,
		               |	ЕСТЬNULL(Сув_Материал.Значение.Наименование, ""null"") КАК Материал,
		               |	ЕСТЬNULL(Сув_Цвет.Значение.Ссылка, ""null"") КАК Цвет,
		               |	ЕСТЬNULL(Сув_ТорговаяМарка.Значение, ""null"") КАК ТорговаяМарка,
		               |	ЕСТЬNULL(Сув_Объем.Значение, ""null"") КАК Объем,
		               |	НоменклатураСпр.Наименование КАК Наименование,
		               |	""/5032292394/Images/"" + НоменклатураПрисоединенныеФайлы.ВладелецФайла.Код + НоменклатураПрисоединенныеФайлы.Наименование + ""."" + НоменклатураПрисоединенныеФайлы.Расширение КАК Изображение
		               |ИЗ
		               |	Справочник.Номенклатура КАК НоменклатураСпр
