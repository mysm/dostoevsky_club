		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ПараметрыFTP.ВыгружатьНаFTP Тогда
					...
					Если МассивДанныхЗадания.Количество() = КоличествоДанныхДляЗадания Тогда
						...
						ЗаписатьВЖурнал(СтрШаблон("Отправка FTP. Задание (%1)(%4). Статус задания %2. Файлы к отправке %3", 
									ФЗ.ИдентификаторЗадания, ФЗ.Статус, ДанныеВJSON(ПараметрыЗадания.МассивДанныхЗадания), ПараметрыЗадания.УникальныйИдентификатор), Настройки);
			...
			Если ПараметрыFTP.ВыгружатьНаFTP Тогда
				Если МассивДанныхЗадания.Количество() > 0 Тогда
					...
					ФЗ = НачатьВыполнениеФоновогоЗадания(ПараметрыЗадания);
					ЗаписатьВЖурнал(СтрШаблон("Отправка FTP. Задание (%1)(%4). Статус задания %2. Файлы к отправке %3", 
								ФЗ.ИдентификаторЗадания, ФЗ.Статус, ДанныеВJSON(ПараметрыЗадания.МассивДанныхЗадания), ПараметрыЗадания.УникальныйИдентификатор), 
								Настройки);
				КонецЕсли;				
			КонецЕсли; 
...
Функция НачатьВыполнениеФоновогоЗадания(ПараметрыВыполненияОбработки)
	...
	Если ПараметрыВыполненияОбработки.Свойство("УникальныйИдентификатор") Тогда
		УникальныйИдентификатор = ПараметрыВыполненияОбработки.УникальныйИдентификатор;
	Иначе	
...
Процедура ВыгрузитьФайлыНаFTP(ПараметрыЗадания, АдресРезультата) Экспорт
	...
	Если ПараметрыЗадания.Свойство("УникальныйИдентификатор") Тогда
		ИмяСобытия = СтрШаблон("%1 (%2).", ИмяСобытия, ПараметрыЗадания.УникальныйИдентификатор);
	КонецЕсли; 
	...
	Если ЗаписыватьВЖурнал Тогда
		...
		ЗаписатьВЖурнал(СтрШаблон("%1 Начало выгрузки. ", ИмяСобытия), Настройки);
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Версия = "2.75";
