...
Функция Выгрузить() Экспорт
	...
	Попытка
		...
		Запрос.УстановитьПараметр("ВсеРодители", Не ЗначениеЗаполнено(ЭтотОбъект.Родитель));
		...
		Иначе
			...
			СтруктураВыгрузки = Новый Структура("books", МассивПозиций); 
...
Функция ВыгрузитьСувениры() Экспорт
	...
	Попытка
		...
		Иначе
			...
			//ВыгружаемыеСправочники.Вставить("souvenir_categories", ЭтотОбъект.Сув_Материал);
...
Функция ИмяФайлаКартинки(НоменлатураСсылка,НоменклатураНаименование)
	...
	ИмяФайлаКартинки = СтрШаблон("pic_%1_%2",Лев(ИмяФайлаКартинки,200),XMLСтрока(НоменлатураСсылка));
